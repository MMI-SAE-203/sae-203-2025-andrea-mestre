...
import { allActiviteByInviteName } from "./backend.mjs";
...
import { modifFilm } from "./backend.mjs";
...
try {
  const activite = await allActiviteByInviteName("Beauvois");
...
try {
  const data = {
    "titre": "Ronaldo le biopic",
  };
  await modifFilm('4c8cu89phb3se13', data);
}
catch (e) {
  console.error(e);
}
